...
namespace nnforge
{
    ...
    bool neural_network_toolset::parse(int argc, char* argv[])
	{
		...
		config.add_options()
			...
			("learning_rate_decay_rate", boost::program_options::value<float>(&learning_rate_decay_rate)->default_value(0.5F), "Degradation of learning rate at each tail epoch, if learning_rate_decay_error is false or not using SGD. Otherwise it is applied if Validating NLL increases.") 
			("learning_rate_decay_sgd_nll", boost::program_options::value<bool>(&learning_rate_decay_sgd_nll)->default_value(false), "If set to true, SGD algorithm will multiply learning rate by learning_rate_decay_rate if Validating NLL increases.")
