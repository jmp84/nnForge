...
#include "supervised_sparse_data_stream_writer.h"
...
supervised_sparse_data_stream_writer::supervised_sparse_data_stream_writer(
  ...
  out_stream->write(
      reinterpret_cast<const char*>(
          supervised_sparse_data_stream_schema::supervised_sparse_data_stream_guid.data),
      sizeof(supervised_sparse_data_stream_schema::supervised_sparse_data_stream_guid.data));
  ...
  out_stream->write(reinterpret_cast<const char*>(&type_code),
                    sizeof(type_code));
...
void supervised_sparse_data_stream_writer::write(
    neuron_data_type::input_type type_code,
  ...
  if (this->type_code == neuron_data_type::type_unknown) {
    ...
    throw neural_network_exception(
        (boost::format("Cannot write elements with different "
            "input type: %1% %2%") % this->type_code % type_code).str());
  }

  out_stream->write(reinterpret_cast<const char*>(input_neurons),
                    input_elem_size * input_sparse_neuron_count);
  out_stream->write(reinterpret_cast<const char*>(output_neurons),
                    sizeof(*output_neurons) * output_sparse_neuron_count);
  entry_count++;
}
...
void supervised_sparse_data_stream_writer::write(
  ...
  if (type_code == neuron_data_type::type_unknown) {
    ...
    throw neural_network_exception(
        (boost::format("Cannot write elements with different input "
            "type: %1% %2%") % type_code % neuron_data_type::type_byte).str());
  }
...
void supervised_sparse_data_stream_writer::write(
  ...
  if (type_code == neuron_data_type::type_unknown) {
    ...
    throw neural_network_exception(
        (boost::format("Cannot write elements with different input "
            "type: %1% %2%") % type_code % neuron_data_type::type_float).str());
  }
...
void supervised_sparse_data_stream_writer::raw_write(
    const void * all_entry_data,
    size_t data_length) {
  if (type_code == neuron_data_type::type_unknown) {
    throw neural_network_exception("Type for input elements is not specified "
        "for supervised_sparse_data_stream_writer");
  }
