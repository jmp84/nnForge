...
public:
   ...
   * output target word-id mappings. Note that the input sizes are for regular
   * words, the constructor will also add special words.
   ...
   * words. No distinction between source and target. This may be extended for
   * more flexibility.
   * @param outputVocabSize The output vocabulary size for regular words.
   */
  Vocab(std::istream& sourceTextFile,
        std::istream& targetTextFile,
        const int inputVocabSize,
        const int outputVocabSize);

  /**
   * Returns the id corresponding to the source word or the special oov id.
   * @param sourceWord The source word in string format.
   * @return The id for the source word if it is in the vocabulary or the oov
   * id.
   */
  const int getInputSourceId(const std::string& sourceWord) const;

  /**
   * Returns the id corresponding to the target word or the special oov id.
   * The difference with getOutputTargetId is that the input vocabulary
   * is considered.
   * @param targetWord The target word in string format.
   * @return The id for the target word if it is in the vocabulary or the oov
   * id.
   */
  ...
  /**
   * Returns the id corresponding to the target word or the special oov id.
   * The difference with getInputTargetId is that the output vocabulary
   * is considered.
   * @param targetWord The target word in string format.
   * @return The id for the target word if it is in the vocabulary or the oov
   * id.
...
private:
  ...
  /** special id for input oovs (no distinction between source and target) */
  int inputOovId_;
  /** special id for output oovs (different than inputOovId_ because the input
   * vocab size and the output vocab size may be different) */
  int outputOovId_;
  ...
  /** special id for target start of sentence (belongs to input vocab; note that
   * it doesn't belong to output vocab because a target start of sentence is
   * never predicted) */
  int targetStartSentenceId_;
  /** special id for target end of sentence (belongs to target vocab; note that
   * it doesn't belong to the input vocab because target end of sentence is
   * never used as context) */
  int targetEndSentenceId_;
  /** input vocab size, including special tokens (oov, <src>, </src>, <trg>) */
  int inputVocabSize_;
  /** output vocab size, including special tokens (oov, </trg>) */
  int outputVocabSize_;
  /** input target vocab offset to get the correct input target ids (in the
   * nnjm paper, source words are in the range 0-15999 and target words are in
   * the range 16000-31999) */
