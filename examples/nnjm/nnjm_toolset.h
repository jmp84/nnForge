...
namespace nnjm
{
	...
	protected:
		 ...
		 * Reads word aligned parallel text and output training data in binary
		 * format to be read by the neural network trainer.
		 */
		 ...
		 * The default in parent class is mse. The default for this application
		 * is cross-entropy.
		 * @return The error function.
		 */
		virtual nnforge::const_error_function_smart_ptr
		get_error_function() const;

		/**
		 * Gets the training data reader. Used for randomizing training data.
		 * Here, the supervised_sparse_data_stream_reader is used.
		...
		/**
		 * Gets the training data writer. Used for randomizing training data.
		 * Here, the supervised_sparse_data_stream_writer is used.
		 * @param reader The training data reader.
		 * @param path The path to the randomized training data.
		 * @return
		 */
		virtual nnforge::data_writer_smart_ptr
		get_randomized_training_data_writer(
				nnforge::supervised_data_reader& reader,
				const boost::filesystem::path& path) const;

		/**
		 * Gets the reader to read training data. The default in parent class is
		 * to use a supervised_data_stream_reader. The default for this
		 * application is to use a supervised_sparse_data_stream_reader. The
		 * difference with get_original_training_data_reader is that the reader
		 * reads randomized data whereas in get_original_training_data_reader,
		 * the reader reads the original training data.
		 * @return The training data reader.
		 */
		virtual nnforge::supervised_data_reader_smart_ptr
	...
	private:
		...
		/**
		 * Computes target to source alignment from an alignment in Berkeley
		 * format.
		 ...
		 * A training instance is a context, i.e. the features, and a target
		 * word, i.e. the label. The context consists of a history of target
		 * words as in a usual n-gram language model and a sequence of source
		 * words.
		 ...
		 * @param target2SourceAlignment The target to source alignment.
		 * @param trainingNgram The resulting training n-gram that consists of
		 * the context (source words and history target words) and the label
		 * (target word) (in the nnjm paper, this is a vector of size 15)
		 */
		 ...
		 * context (source words and history target words) and the label (target
		 * word) (in the nnjm paper, this is a vector of size 15)
		 * @param inputSparseData The input data to be fed to the training
		 * writer (in the nnjm paper, this is a vector of size 14, if it was not
		 * sparse, it would be a vector of size 14 * 16000).
		 */
		void convertToInputSparseData(
		 ...
		 * context (source words and history target words) and the label (target
		 * word) (in the nnjm paper, this is a vector of size 15)
		 * @param outputData The output data to be fed to the training writer
		 * (in the nnjm paper, this is a vector of size 1, if it was not sparse,
		 * it would be a vector of size 32000).
		 */
		void convertToOutputSparseData(
				const std::vector<WordId>& trainingNgram,
